    ...
    return (
        <>
            <div className=" border-b pb-6">
                <div className=" gap-3 flex container mx-auto  md:px-24 px-4">
                    <div
                        className={`text-green-200 text-xl font-semibold uppercase px-4 py-2.5 rounded-[100px] cursor-pointer ${selectedTab === 'all' ? 'bg-gray-100' : 'bg-green-300'
                            }`}
                    ...
                    {locations.map((location) => (
                        <div
                            key={location.name}
                            className={`px-4 py-2.5 rounded-[100px] flex-col justify-center items-center gap-2.5 inline-flex cursor-pointer ${selectedTab === location.name ? 'bg-gray-100' : 'bg-green-300'
                                }`}
                            onClick={() => handleTabClick(location.name)}
                        >
                            <div className="justify-end items-center gap-2.5 inline-flex">
                                <div className="justify-center items-center gap-1 flex">
                                    <div className={`text-green-200 text-xl font-semibold uppercase`}>
                                        {location.label}
                                    </div>
                                </div>
                            </div>
            ...
            <div className="container mx-auto  md:px-24 px-4 md:pt-12 md:pb-24">
                ...
                <div className='flex gap-x-6 gap-y-6 flex-wrap'>
                    {filteredPlaces.map((place, index) => (
                        <div key={index} className={` w-full md:w-666 rounded-3xl py-5 px-6 ${place.branch ? 'bg-green-300' : ' border border-gray-50'}`}>
                            <span className={`  rounded-s px-2 text-base font-semibold ${place.branch ? 'bg-green-400 text-white' : ' bg-[#F9A000] text-black'}`}> {place.type}</span>
                            <p className='text-green-200 text-xl font-semibold mt-2 mb-1'>{place.place_name}</p>
                            <p className=' text-gray-50 text-base'>{place.details}</p>
                            <div className='flex mt-3'> 
                                <a href={`tel:${place.number}`} className='flex text-green-400 text-base font-semibold gap-x-2'>
