            ...
            <div className={`${thanks ? 'md:min-w-[500px]' : 'md:min-w-[470px]'} w-full`} >
                ...
                <div className="bg-white shadow-sm  md:py-8 py-4  rounded-[32px] overflow-hidden" style={{ boxShadow: ' 0px 20px 66px rgba(0, 0, 0, 0.2)' }}>
                    <h2 className="  hidden md:flex md:text-[32px]  font-bold capitalize md:px-10 px-4 md:pb-6  pb-4 border-b border-grayy-100 justify-between">
                        ...
                        {thanks ? '' :
                            <Link to="/cart" className=" py-2 px-4  border-2 border-gray-400 text-xl text-green-200 rounded-100">
                                تعديل
                            </Link>
                        }
                    ...
                    <div className={` md:block ${opencart ? 'block' : 'hidden'} `}>
                        <ul role="list" className=" md:mx-10 mx-4 border-b border-gray-200">
                            {cartItems.map((item) => (
                        ...
                        <div className="points flex items-center bg-yellow-910 py-4 md:px-10 px-4 justify-between mt-6">
                        ...
                        <dl className="px-4 py-6 space-y-6  sm:px-6">
                            <div className="flex items-center justify-between">
                                ...
                                <dd className="text-black text-xl font-bold">{FormatCurrency2(totalSubTotal)}</dd>
                            </div>
                            <div className="flex items-center justify-between">
                                <dt className=" font-semibold text-gray-50 text-bas">مصاريف التوصيل</dt>
                                <dd className="text-black text-xl font-bold">{shippingFees !== 0 ? FormatCurrency2(shippingFees) : 'توصيل مجاناً'}</dd>
                            </div>
                            {/* <div className="flex items-center justify-between">
                            ...
                            {totalDiscountAPI > 0 ?
                                <div className="flex items-center justify-between">
                                    ...
                                    <dd className="text-sm font-medium text-red-400">-{FormatCurrency2(totalDiscountAPI)}</dd>
                                </div>
                                :
                                ''
                            }
                            <div className="flex items-center justify-between pt-6 border-t border-gray-200">
                                <dt className="font-semibold text-gray-50 text-base">الإجمالي</dt>
                                <dd className=" text-[32px] text-black font-bold">{FormatCurrency2(totalAmount)}</dd>
                            </div>
                            {thanks ?
                                <div>
